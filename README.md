...
| [moon-proto-monorepo](moon-proto-monorepo/SKILL.md)     | Opinionated, language-agnostic [moon](https://moonrepo.dev) **v2** + [proto](https://moonrepo.dev/docs/proto) build-tooling config for polyglot monorepos. Owns workspace globs and project discovery, toolchain pinning and the proto↔moon seam, task definition and `inheritedBy` inheritance, env loading, caching (`inputs`/`outputs` hygiene), CI, and codegen. Closed-set value tables, v1→v2 rename map, a forbidden/anti-pattern list, decision flowcharts, and a `scripts/moon-checks.sh` verifier (exit 0/1/2/3) that detects drift on any moon repo. Worked examples are a fictional polyglot repo (Go/Rust/Python). Complements `go-modularization` (folder layout); does not overlap.                                |
