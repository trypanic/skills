...
description: Opinionated folder/package layout for Go projects (monorepos and single-service repos). Encodes pragmatic flat hexagonal architecture (inbound adapters → interactor → domain/ports ← outbound adapters), suffix-then-folder promotion for API versions and bounded contexts, the `go-pkgs/` (never `pkg/`) shared-utility convention, the `data_repositories/` vs `storage/` shape split, the migrations grammar with closed verb set, and the forbidden folder-name list. **Both users and AI agents must consult this skill before placing new code, modularizing a feature, or restructuring.** Use when the user or agent says "new Go service", "scaffold service folders", "where does this file go", "promote to subfolder", "add a new adapter", "structure the monorepo", "set up migrations folder", "review folder layout", "is this folder name allowed", or any phrase asking to create, place, rename, or restructure code under `services/<service>/`, `go-pkgs/`, `internal/`, or `migrations/`. Auto-trigger when a new top-level folder is created under `services/<service>/`, when a new file is placed under `go-pkgs/` or `internal/`, when a migration file is added, when a refactor moves code across packages, or when a PR review touches folder layout. Do NOT trigger for non-Go projects, or for runtime/observability/lint config questions (out of scope).
---
