...
| R-24 | Two interactor shapes (superseded) | **Superseded by R-34 / ADR-34.** `interactor/` no longer houses process managers — it holds use cases only; process managers live in the `coordinator/` inner layer. Row kept for stable numbering; see R-34.                                                                                                       |
...
| R-34 | Use cases vs coordinators (two layers) | `interactor/` holds **use cases only** — Clean Architecture interactors: application-specific business rules, one workflow step, ~1–3 port calls, no goroutines (`interactor_<context>.go`). Process managers (loops/goroutines/mutexes/timers coordinating ports and use cases over time) live in the **`coordinator/`** inner layer, role-named with no layer prefix (`coordinator/scheduler.go`, never `coordinator_scheduler.go`/`interactor_scheduler.go`); one role-naming style per service. Direction: `cmd`/inbound adapters → `coordinator` → `interactor` → `domain`/`ports`; `interactor` never imports `coordinator`. `coordinator/` obeys all inner-layer import invariants (no adapters, no generated contracts). Both layers flat until ≥10-file promote. Supersedes R-24. |
...
```text
cmd                                    → all (wiring only)
<inbound_adapter>                      → coordinator, interactor, internal/contracts (service-scoped)
  e.g. api / consumer / cli
coordinator                            → interactor, domain, ports, internal/contracts (service-scoped)
...
- `<layer>` (`domain`, `ports`, `interactor`, `coordinator`) importing any adapter folder — incl. a streaming server (`grpc/`/`ws/`/`sse/`).
- `<layer>` (`domain`, `ports`, `interactor`, `coordinator`) importing a generated wire contract (versioned `internal/contracts/**/v<N>` or any `*.pb.go`) — adapter-only (R-25).
- `interactor` (or `domain`/`ports`) importing `coordinator` — the dependency points use-case-ward (R-34).
